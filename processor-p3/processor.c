...
//      cdbs_print("start of tick");
        // The register file is written via the result bus. (SU g + set CDB.busy to false)
        for (int j = 0; j < C; j++) {
            if (buses[j].busy) {
                if (buses[j].reg_id >= 0) {
        ...
        // The state update unit pulls from the state update queue and updates the 
        // result bus (SU a-e)
        for (int c = 0; c < C; c++) {
            if (queue_empty(state_update_queue)) break;
            ...
            // Scoreboard[FU].busy = false already done by setting FU_pipeline[j][0] null
...
//          DPRINTF("to_queue: %p\n", to_queue);
        ...
        for (int j = 0; j < 2; j++) {
            ...
            while (cur_node != NULL) {
                ...
                if (!RS->fired) {
                    bool ready = true;
                    for (int j = 0; j < 2; j++) {
                        if (RS->src_arr[j] != NULL && !RS->src_arr[j]->ready) ready = false;
                    }
                    if (ready) {
                        // find next possible FU
                        for (int j = 0; j < J + K; j++) {
                            if (j < J && RS->is_long) continue;
                            if (j >= J && !RS->is_long) continue;
        ...
        while (cur_node != NULL) {
            if (queue_full(long_schedule_queue) && queue_full(fast_schedule_queue)) {
...
//              queue_print("fast after", fast_schedule_queue);
            ...
            bool cleared = cur_instr->src_arr[0] == NULL ||
                                    cur_instr->src_arr[1] == NULL;
...
//              break;
            ...
            case ALU_LONG:;
                ...
                DPRINTF("push %p into dispatch queue\n", new_instr);
...
//              queue_print("dispatch_queue", dispatch_queue);
        ...
        for (int c = 0; c < C; c++) {
            ...
            if (del_instr != NULL) {
                ...
                progress = 1;
                instr_queue *q = del_instr->is_long ? 
                    long_schedule_queue : fast_schedule_queue;
